...
vjs.getAttributeValues = function(tag){
  ...
  knownBooleans = ','+'cast,autoplay,controls,loop,muted,default'+',';
...
vjs.Component = vjs.CoreObject.extend({
  ...
  init: function(player, options, ready){
    ...
    //no poster if flash block is activated
...
//BJA document.getElementBy vervangen
...
vjs.Player.prototype.techGet = function(method){
  ...
  if (this.tech && this.tech.isReady_) {
	...
	if (this.techName== "Flash" ){
		if(method=="paused"){
			return ( this.tech.el_.getState() == "paused" );
		}
...
vjs.Player.prototype.currentTime = function(seconds){
  ...
  if( this.techName == "Flash" ) {
	return this.cache_.currentTime = (player.getCurrentTime() || 0);
  } else {
...
vjs.Player.prototype.volume = function(percentAsDecimal){
  ...
  if( this.techName == "Flash"){
  	if( this.tech.isReady_) {
    	vol = parseFloat(player.getVolume(vol));
    }
  } else {
...
vjs.Player.prototype.muted = function(muted){
  ...
  if( this.techName == "Flash"){
	if( this.tech.isReady_) {
      return player.getMuted() || false; // Default to false
    }
  } else {
    ...
    if (bool !== this.userActive_) {
      ...
      if (bool) {
        ...
        // https://code.google.com/p/chromium/issues/detail?id=103041
  		...
  		if( this.techName == "Flash"){
          //BJA WAT TE DOEN HIER MET FLASH? strobe ondersteunt geen 'mousemove'
        } else {
...
//BJA
  return (vjs.IS_SAFARI || vjs.IS_IOS || vjs.IS_ANDROID ) && !( vjs.IS_FIREFOX || ( vjs.IS_CHROME && !vjs.IS_ANDROID) );
...
//          'preload': playerOptions.preload,
    ...
    if (options['iFrameMode'] === true && !vjs.IS_FIREFOX) {
      ...
      vjs.on(iFrm, 'load', vjs.bind(this, function(){
        ...
        document.getElementById('vjs-poster-id').style.display="None"
...
org.strobemediaplayback.StrobeMediaPlaybackJSUI = function(player)
{
	this.player = player;
}
...
org.strobemediaplayback.StrobeMediaPlaybackJSUI.prototype = 
{
	onDurationChange: function(duration){
		vjs.trigger(player,'durationchange',null);
	},
	
	onCurrentTimeChange: function(currentTime){
		vjs.trigger(player,'timeupdate',null);
	},

	onVolumeChange: function(value)	{
		vjs.trigger(player,'volumechange',null);
	},
	
	onMutedChange : function(value)	{
		vjs.trigger(player,'volumechange',null);
	},
	onMediaPlayerStateChange : function(state){
		if (state == "ready") {
			if(this.old_state == "loading") {
				videojs.Flash.onReady(player.id);
			}
			if(this.old_state == "buffering") {
				//BJA Flash player delay before we start it. fix this in FLASH
				player.play2();
		...
		if (state == "playing" ) {
			//BJA loop does not work in flash, this is a hack which makes it work
			//Loop works, but videojs thinks player is paused. Even player.getstate() gives pause: bug in flash
			//detect stop, should be "state == stopped"...
			if(player.getCurrentTime() >= (player.getDuration()-0.1)){
				if(player.getLoop()){
					player.pause();
					player.setCurrentTime(0);
					vjs.trigger(player,"play",null);
				} else {
					redirect_at_end();
				}
			} else {
		...
		if (state == "paused" ) {
			...
			if(this.old_state == "buffering") {
				//BJA Flash player delay before we start it. fix this in FLASH
				//player.play2();
				vjs.trigger(player,"pause",null);
				//alert("lala");
				//setTimeout('vjs.trigger(player,"play",null);', 1000)
			} else {
				vjs.trigger(player,"pause",null);
			}
		}
...
function onJavaScriptBridgeCreated(id){
	if (player == null){
		...
		org.strobemediaplayback.players[id] = new org.strobemediaplayback.StrobeMediaPlaybackJSUI(player);
		org.strobemediaplayback.bindListeners(player, org.strobemediaplayback.players[id], "org.strobemediaplayback.players['" + id + "']");
		flash_loaded=true;
		document.getElementById('vjs-poster-id').style.display=""
//why turned off?
//		vjs.PosterImage.prototype.createPosterEl();
	}				
}
...
flashBlockDetect = function(callbackMethod){
				...
				if(navigator.userAgent.indexOf('Firefox') > -1 ){
					if (document.getElementById('FlashBlock') == null) {
					   return_value = 1;
					}
...
function callbackFlashblockDetectMethod(val) {
  if (val > 0) {
    setTimeout(function(){
    document.getElementById('vjs-poster-id').style.display="None"
...
vjs.Flash.embed = function(swf, placeHolder, flashVars, params, attributes){
  ...
  flashBlockDetect(callbackFlashblockDetectMethod);
//If flashbocker detected, do not display poster
//  setTimeout(function(){
//    document.getElementById('vjs-poster-id').style.display="None"
    newObj.style.display = 'block';
//  }, 1000);
 ...
 * @constructor
 */
vjs.CastButton = vjs.MenuButton.extend({
  /** @constructor */
  init: function(player, options, ready){
//    if (this.options_['cast']) {
		vjs.Button.call(this, player, options);
//    }

//    this.el_.id = 'cast-button';
  }
});
vjs.CastButton.prototype.kind_ = 'cast';
vjs.CastButton.prototype.buttonText = 'Cast';
vjs.CastButton.prototype.className = 'vjs-cast-button';
vjs.CastButton.prototype.onClick = function(){
    try {
  		showCastModal();
    }
    catch(err) {
...
videojs.options.flash.swf = "/static/flash/StrobeMediaPlayback.swf";